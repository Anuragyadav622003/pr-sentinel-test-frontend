...
function ProviderSwitcher({ mode, configs, onModeChange, onSelect }: { mode: "FREE" | "BYOK"; configs: ProviderConfig[]; onModeChange: (mode: "FREE" | "BYOK") => void; onSelect: (provider: LlmProviderType) => void }) {
  ...
  if (mode === "FREE") return <div className="reviewer-pill"><span className="pulse-dot" /><div><strong>PR Sentinel free AI</strong><small>3/5 reviews left today</small></div><div className="reviewer-progress"><span style={{ width: "40%" }} /></div><button type="button" onClick={() => onModeChange("BYOK")}>Switch to my own key</button></div>;
...
function AiSettings() {
  ...
  return <div className="ai-settings-layout">
    ...
    <section className="workspace-panel ai-mode-panel"><div className="panel-header"><div><p className="eyebrow">REVIEW ENGINE</p><h2>AI review mode</h2><p>Choose how PR Sentinel powers automated reviews.</p></div><span className="status-badge info">{mode === "FREE" ? "Free tier" : "BYOK active"}</span></div><div className="mode-options">
      <button className={`mode-option ${mode === "FREE" ? "selected" : ""}`} onClick={() => setMode("FREE")} aria-pressed={mode === "FREE"}><span className="radio-dot" /><span><strong>PR Sentinel free AI</strong><small>We run reviews on a shared model. No API key needed. Limited to 5 reviews per day.</small></span><span className="mode-tag">3 left</span></button>
    ...
    <section className="workspace-panel quota-panel"><div className="quota-heading"><div><p className="eyebrow">USAGE</p><h2>Free AI allowance</h2></div><strong>2 <small>/ 5 reviews today</small></strong></div><div className="quota-track"><span style={{ width: "40%" }} /></div><p>Resets daily at midnight UTC. Switch to your own key when you need more capacity.</p></section>
  </div>;
}
